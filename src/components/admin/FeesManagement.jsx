...
import { Container, Row, Col, Card, Table, Button, Form, Modal, Alert, Spinner, Badge, Tabs, Tab } from 'react-bootstrap';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faPlus, faEdit, faTrashAlt, faCheckCircle, faTimes, faFilter } from '@fortawesome/free-solid-svg-icons';
...
const FeesManagement = () => {
  ...
  const feeTypes = ['All', ...new Set(mockFeeStructures.map(fee => fee.type))];
    ...
    <Container fluid className="py-4">
      <Row className="mb-4">
        <Col>
          ...
          <p className="text-muted">Manage fee structures, amounts, and due dates</p>
        </Col>
        <Col xs="auto">
          <Button 
            variant="primary" 
                  ...
                  <div className="d-flex gap-2">
                    ...
                    >
                      {feeTypes.map((type, index) => (
                        <option key={index} value={type}>{type === 'All' ? 'All Fee Types' : type}</option>
                      ))}
                    </Form.Select>
                ...
                <Table hover className="align-middle mb-0">
                  ...
                  <tbody>
                    {filteredFeeStructures.length === 0 ? (
                      <tr>
                        <td colSpan="9" className="text-center py-4">No fee structures found</td>
                      </tr>
                    ) : (
                      filteredFeeStructures.map(fee => (
                        <tr key={fee.id}>
                          ...
                          <td>{fee.branch}</td>
                          <td>{fee.semester}</td>
                          <td className="fw-medium">{formatCurrency(fee.amount)}</td>
      ...
      >
        ...
        <Modal.Body>
          <p>Are you sure you want to delete the fee structure for <strong>{currentFee?.type}</strong>?</p>
